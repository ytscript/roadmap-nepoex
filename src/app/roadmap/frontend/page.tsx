...
export default function FrontendRoadmap() {
  ...
  const roadmapItems = useRoadmapStore(state => state.frontendCards) // frontend kategorisi
  ...
  useEffect(() => {
    async function loadRoadmapData() {
      ...
      try {
        setIsLoading(true)
        const cards = await fetchRoadmapCards('frontend') // frontend kategorisi
        ...
        if (user) {
          ...
          setCards('frontend', updatedCards) // frontend kategorisi
        } else {
          setCards('frontend', cards) // frontend kategorisi
        }
      } catch (error) {
  ...
  const handleTopicStatusChange = async (itemId: string, topicId: string, completed: boolean) => {
    ...
    try {
      ...
      updateTopicStatus('frontend', itemId, topicId, completed) // frontend kategorisi

      if (selectedItem?.id === itemId) {
        const updatedItem = getCards('frontend').find(item => item.id === itemId) // frontend kategorisi
        if (updatedItem) {
    ...
    <main className="min-h-screen bg-[#1a1a2e] text-white pt-24 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          >
            Modern web geliştirme yolculuğunda ilerleyerek bir frontend geliştirici olun! 🚀
          </motion.p>
