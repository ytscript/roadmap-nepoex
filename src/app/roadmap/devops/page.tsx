...
export default function DevOpsRoadmap() {
  ...
  const roadmapItems = useRoadmapStore(state => state.devopsCards)
  ...
  useEffect(() => {
    async function loadRoadmapData() {
      ...
      try {
        setIsLoading(true)
        const cards = await fetchRoadmapCards('devops') // Kategori değişti
        ...
        if (user) {
          ...
          setCards('devops', updatedCards) // Kategori değişti
        } else {
          setCards('devops', cards) // Kategori değişti
        }
      } catch (error) {
  ...
  // Topic durumunu güncelleme - kategori değişti
  const handleTopicStatusChange = async (itemId: string, topicId: string, completed: boolean) => {
    ...
    try {
      ...
      updateTopicStatus('devops', itemId, topicId, completed) // Kategori değişti

      if (selectedItem?.id === itemId) {
        const updatedItem = getCards('devops').find(item => item.id === itemId) // Kategori değişti
        if (updatedItem) {
  ...
  // Return kısmı - sadece başlık ve açıklama değişti
  return (
    <main className="min-h-screen bg-[#1a1a2e] text-white pt-24 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          >
            Modern DevOps pratiklerini öğrenerek bir DevOps mühendisi olun! 🚀
          </motion.p>
