...
export default function MobileRoadmap() {
  ...
  const roadmapItems = useRoadmapStore(state => state.mobileCards) // mobile kategorisi
  ...
  useEffect(() => {
    async function loadRoadmapData() {
      ...
      try {
        setIsLoading(true)
        const cards = await fetchRoadmapCards('mobile') // mobile kategorisi
        ...
        if (user) {
          ...
          setCards('mobile', updatedCards) // mobile kategorisi
        } else {
          setCards('mobile', cards) // mobile kategorisi
        }
      } catch (error) {
  ...
  const handleTopicStatusChange = async (itemId: string, topicId: string, completed: boolean) => {
    ...
    try {
      ...
      updateTopicStatus('mobile', itemId, topicId, completed) // mobile kategorisi

      if (selectedItem?.id === itemId) {
        const updatedItem = getCards('mobile').find(item => item.id === itemId) // mobile kategorisi
        if (updatedItem) {
    ...
    <main className="min-h-screen bg-[#1a1a2e] text-white pt-24 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          >
            Modern mobil uygulama geliştirme yolculuğunda ilerleyerek bir mobil geliştirici olun! 🚀
          </motion.p>
